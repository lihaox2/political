	...
	public ResponseEntity<?> importHealthyExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (policeId != null && !"".equals(policeId)) {
				...
				if (user == null || user.getName() == null) {
					jsonObjectList.add(errorJsonObjCreate(i, excel.get(5), "该警员不存在"));
					continue;
				}
			}else {
				jsonObjectList.add(errorJsonObjCreate(i, excel.get(5), "警号未填写"));
				continue;
			}
	...
	public ResponseEntity<?> importSchoolExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (policeId != null && !"".equals(policeId)) {
				...
				if (user == null || user.getName() == null) {
					jsonObjectList.add(errorJsonObjCreate(i, excel.get(3), "该警员不存在"));
					continue;
				}
			}else {
				jsonObjectList.add(errorJsonObjCreate(i, excel.get(3), "警号未填写"));
				continue;
			}
	...
	public ResponseEntity<?> importUnitTrainExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (policeId != null && !"".equals(policeId)) {
				...
				if (user == null || user.getName() == null) {
					jsonObjectList.add(errorJsonObjCreate(i, excel.get(2), "该警员不存在"));
					continue;
				}
			}else {
				jsonObjectList.add(errorJsonObjCreate(i, excel.get(2), "警号未填写"));
				continue;
			}
	...
	public ResponseEntity<?> importDutyCaseExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (informationId == null) {
				jsonObjectList.add(errorJsonObjCreate(i, policeId, "警情类别填写错误，请填写系统已有的警情类别"));
				continue;
			}
			if (errorId == null) {
				jsonObjectList.add(errorJsonObjCreate(i, policeId, "错误类别填写错误，请填写系统已有的错误类别"));
				continue;
			}
	...
	public ResponseEntity<?> importCaseLawExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (policeId != null && !"".equals(policeId)) {
				...
				if (user == null || user.getName() == null) {
					jsonObjectList.add(errorJsonObjCreate(i, excel.get(0), "该警员不存在"));
					continue;
				}
			}else {
				jsonObjectList.add(errorJsonObjCreate(i, excel.get(0), "警号未填写"));
				continue;
			}
			...
			if (parentId == null) {
				jsonObjectList.add(errorJsonObjCreate(i, policeId, "问题分类填写错误，请填写系统中已有的问题分类"));
				continue;
			}
			typeId = riskCaseLawEnforcementTypeService.findByNameAndParentId(excel.get(5), parentId);
			if (typeId == null) {
				jsonObjectList.add(errorJsonObjCreate(i, policeId, "扣分标准填写错误，请填写系统中已有的扣分标准"));
				continue;
			}
	...
	public ResponseEntity<?> importAbilityExcel(@Param("file") MultipartFile file) throws Exception {
		...
		for (List<String> excel : readExcel) {
			...
			if (columnDate != null && !"".equals(columnDate)) {
				...
				jsonObjectList.add(errorJsonObjCreate(i, policeId, "记录时间未填写"));
				continue;
			}
